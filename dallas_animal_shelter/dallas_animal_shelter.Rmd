...
Our intentions with this data set is to answer a multitude of different questions that could help the animal shelter in the future. I think the following questions can help the animal shelter optimize a plan for incoming pets and try to maximize the opportunity for a pet to get adopted:
...
This proved that the `simple` tab is just a subset of the `raw` tab. Since the `raw` data is not overly large, I will just use the `raw` tab and filter out columns that really won't help us. This way I will avoid missing any conclusions that can be drawn from the `raw` tab information that cannot be drawn from the `simple` tab. Below is a `glimpse` of the Dallas Animal Shelter columns which seemed potentially meaningful form the `raw` tab.
...
* I added `total_time` which represents the number or days of the animal was at the shelter.
...
In our analysis, I broke in down in three different "events". Intake, Time at Shelter, and Outcome. These sections represent the flow of an animal coming into the shelter, the time at the shelter, and then leaving the shelter.
...
The other big spike before Labor Day is somewhat unexplained. Our guess is that they had some sort of big promotion. I am pretty sure the humane society in my town does a promotion for free or $1 dogs at some point in the summer.
